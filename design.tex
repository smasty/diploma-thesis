...
\item{
    ...
    s informačným bulletinom a model používateľa sa bude na jej základe aktualizovať.
}
\end{my_itemize}
...
Model modelujúci expertízu používateľa sa bude skladať z otázok, ktoré používateľ označil za obľúbené alebo
na ktoré používateľ odpovedal, pričom ich dopad na model expertízy bude závislý od skóre jeho odpovede.
Nezáporné skóre bude prispievať do modelu pozitívne -- bude to teda signalizovať
...
\textbf{Spätná väzba}\\
Do oboch modelov budú využitím rovnakého princípu prispievať aj implicitná a explicitná spätná väzba používateľa --
...
\textbf{Vzorce pre zostavenie záujmového a expertízneho modelu používateľa}\\
...
V prípade nových používateľov, ktorí v systéme nemajú žiadnu, alebo iba nedostatočnú aktivitu, sa zo začiatku namiesto modelu
konkrétneho používateľa použije komunitný model, ktorý je zostavený analogicky k modelu používateľa, no pozostáva
...
\begin{my_enumerate}
\item{Pre každého používateľa vyberieme náhodne z jeho modelu $\left\lceil\frac{n}{2}\right\rceil$~značiek
